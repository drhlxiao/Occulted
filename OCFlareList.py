...
class OCFlareList(object):
    ...
    def __init__(self, IDs=True, folder=False, freload=False,calc_times=False, calc_missing=False,gen=False,filename=False,from_csv=False):
        ...
        if IDs and type(IDs) == list:
            ...
            for i in IDs:
                if type(i) == OCFlare.OCFlare:
                    self.list.append(i)
                elif os.path.isfile(str(i)+'OCFlare.p') and not freload:
...
###################################################  LIST WIDE CALCULATIONS ##########################################################
    ...
    def which_stereo_py(self,retwrong=False):
        ''' uses existing coords to calculate which stereo, using theta_obs routines'''
        import theta_obs as th
        bad=[]
        for f in self.list:
            mlonlat=f.Observation.mlonlat
            allx,ally=f.Observation.Alonlat
            bllx,blly=f.Observation.Blonlat

            v2=(np.deg2rad(ally),np.deg2rad(allx))
            vec2=(np.cos(v2[0])*np.cos(v2[1]),np.cos(v2[0])*np.sin(v2[1]))
            s1E,s1W=senkret(mlonlat)
            s2E,s2W=senkret(vec2)
            Athobs,_,_,_,Afov=th.theta_obs(vec1,s1E,s1W,vec2,s2E,s2W)

            v2=(np.deg2rad(blly),np.deg2rad(bllx))
            vec2=(np.cos(v2[0])*np.cos(v2[1]),np.cos(v2[0])*np.sin(v2[1]))
            s1E,s1W=senkret(mlonlat)
            s2E,s2W=senkret(vec2)
            Bthobs,_,_,_,Bfov=th.theta_obs(vec1,s1E,s1W,vec2,s2E,s2W)

            if Athobs < Bthobs:
                f.Observation.which_stereo_recalc='B'
            else:
                f.Observation.which_stereo_recalc='A'
            if retwrong and f.Observation.which_stereo_recalc != f.Observation.STEREO:
                bad.append(f)
        if retwrong:
            return bad
    ...
    def cHEK_all(self):
        ...
        for f in self.list:
            f.cHEK_flare()
            if 'HEK_instr' in f.Observation.__dict__.keys():
                counter +=1
                newlist.append(f)
        return newlist
...
####################################################  PLOT METHODS  ##################################################################
    ...
    def plot_loc(self, goes=False, aia=True, stereo=True, ltc=True):
      '''plot flare locations on disk as seen from Earth '''
      import astropy.units as u
      from astropy.coordinates import SkyCoord
      rsun_km=sunpy.sun.constants.radius.to('km')
      heklist=[f for f in self.list if 'HEK_instr' in f.Observation.__dict__.keys()]
      euvilist=[f for f in self.list if 'euvi_event_props' in f.Properties.__dict__.keys()]
      if goes:
          heklist=[h for h in heklist if h.Observation.HEK_instr[0] == 'GOES']
      if aia:
      ...
      for h in heklist:
          ...
          x=float(loc[loc.find('(')+1:loc.find(' ')])
          y=float(loc[loc.find(' ')+1:loc.find(')')])
          try:
              ratio=h.Properties.Messenger_GOES_TEM_long/h.Properties.GOES_long_int
          ...
          if y >= -900:
              ax.scatter(x,y,marker='o',s=ratio*10,color=color)
          if ltc and h.Properties.GOES_long_int <= 2e-6 and y >=-900:
              ax.scatter(x,y,marker='+',s=100)
          #plot limb?
      if stereo:
          for euv in euvilist:
              h=euv.Properties.euvi_event_props
              edt=euv.Datetimes.Messenger_peak
              if type(edt) == str:
                  euv.Datetimes.convert2datetime()
                  edt=euv.Datetimes.Messenger_peak
              euvi_hgs=SkyCoord(h['coords'][0]*u.deg,h['coords'][1]*u.deg, frame='heliographic_stonyhurst',obstime=edt).represent_as('cartesian')
              ratio=euv.Properties.Messenger_GOES/euv.Properties.GOES_GOES_long
              if ratio > 1.0:
                  color='c'
              else:
                  color='m'
      ...
      limb=plt.Circle((0,0),959,fill=False)
      ax.add_artist(limb)
      ax.set_aspect('equal')
      ...
      labels=['<C2','1x','5x','10x']
      cross, = plt.plot([], "k+", markersize=8) #various artists
      dot5, = plt.plot([], "co", markersize=1) #various artists
      dot10, = plt.plot([], "co", markersize=5) #various artists
      dot100, = plt.plot([], "co", markersize=10) #various artists
      handles=[cross,dot5,dot10,dot100]
      ax.legend(handles, labels,numpoints=1)#add legend with dot sizes shown in proportion to 5,10,100x etc
      #why are there 2 of them? that I don't get
      fig.show()
    ...
    def plot_goes_messenger(self,title= "",ymin=10**-9, ymax=10**-3, minangle=False,maxangle=False,labels=0,loglog=True, loc=True, scatter = True,cc='GOES',save=False,show=True, channel='short', line=False, linexlim=False, from_T_EM=False,xmin=10**-8,xmax=10**-3, useHEK=False, gcounts=False,swpc=False,gint=False):
        '''make a plot of the Messenger_GOES vs GOES_GOES'''
        import matplotlib.patches as mpatches
        from datetime import datetime

        if from_T_EM and 'Messenger_GOES_TEM_long' not in self.list[0].Properties.__dict__.keys():
            ...
            if channel=='short':
                mc=mcs
                for f,m in zip(self.list,mc):
                    f.Properties.Messenger_GOES_TEM_short=m
            else:
                mc=mcl
                for f,m in zip(self.list,mc):
                    f.Properties.Messenger_GOES_TEM_long=m

        elif from_T_EM:
            if channel == 'short':
                mc=self.isolate_att('Messenger_GOES_TEM_short')
            else:
                mc=self.isolate_att('Messenger_GOES_TEM_long')
        else:
            #mc=self.isolate_att("Messenger_GOES_long") #peak counts/s
            if channel == 'long':
                mc=self.isolate_att("Messenger_counts_lo") #2/20 switch from hi to lo
            else:
                mc=self.isolate_att("Messenger_counts_hi") #2/20 swith from hi to lo
        ...
        if channel == 'short':
            ...
            if swpc:
                gc=np.array(gc)/0.85
            if gint:
                gc=self.isolate_att('GOES_short_int')
            if gcounts:
                gc=self.isolate_att("GOES_counts_short")
        else:
            if useHEK:
                gc=self.isolate_att('HEK_gc') #should be all right cuz checks Properties before observation
            else:
                gc=self.isolate_att("GOES_GOES_long")
                if swpc:
                    gc=np.array(gc)/0.7
                if gint:
                    gc=self.isolate_att('GOES_long_int')
                if gcounts:
                    gc=self.isolate_att("GOES_counts_long")
...
#             return line, x,yfit(x),res,fdatax,fdatay
        ...
        if line:
            #lparams=self._line_of_best_fit(gcgt0,mcgt0,minx=linexlim[0],maxx=linexlim[1])
            lparams,lx,ly,err,fdatax,fdatay = self._line_of_best_fit(gcgt0,mcgt0,minx=linexlim[0],maxx=linexlim[1])
            #print l
        ...
        if line:
            ...
            lps=None
        return gcgt0,mcgt0,lps#fdatax,fdatay #return line of best fit if it exists

    def scat_a_b(self,akey,bkey,facA=False,facB=False, log=True,xlim=[1e-6,1e-3],ylim=[1e-6,1e-3],xlabel=False,ylabel=False,nonzero=False, cvec=False, cmap='seismic',cran=False, dashed=False,aspect=False):
        ...
        if nonzero: #problem! if these are masked at different points, these will not line up correctly!!!
            mavec=np.ma.masked_where(avec == 0.0,avec)
            mbvec=np.ma.masked_where(bvec == 0.0,bvec)
            avec=mavec.flatten()
            bvec=mbvec.flatten()
        if type(cvec) != bool:
            cvecl=self.isolate_att(cvec)

        #deal with nans. change floats to np.nan

        fig,ax=plt.subplots()
        ax.scatter(avec,bvec)
        if aspect:
            ax.set_aspect('equal')
        if type(cvec) != bool: #this is the vector to color code by
            cbar=ax.scatter(np.array(avec),np.array(bvec),s=50,c=np.array(cvecl),cmap=cmap)
        else:
            ax.scatter(np.array(avec),np.array(bvec),c='r',marker='v',s=60)
        if dashed:
            xvec=np.linspace(xlim[0],xlim[1],10)
            ax.plot(xvec,xvec,'--k')
        ax.set_xlim(xlim)
        ax.set_ylim(ylim)
        if not xlabel:
    ...
    def _line_of_best_fit(self,datax,datay,loglog= True,minx=False,maxx=False,minratio=False,maxratio=False):                 #if list convert to np.arra
        fdatax,fdatay=[],[]
    ...
    def plot_ospex_fit_params(self):
        t1=self.isolate_att('T1')
        t2=self.isolate_att('T2')
        em1=self.isolate_att('EM1')
        em2=self.isolate_att('EM2')
        free_abund=self.isolate_att('free_abund')
        fig,ax=plt.subplots()
        ax.scatter(range(0,len(self.list)),t1,c='r',label='T1')
        ax.scatter(range(0,len(self.list)),em1,c='m',label='EM1')
        ax.scatter(range(0,len(self.list)),t2,c='b',label='T2')
        ax.scatter(range(0,len(self.list)),em2,c='g',label='EM2')
        ax.scatter(range(0,len(self.list)),free_abund, c='y',label='abund')
        ax.scatter(range(0,len(self.list)),np.zeros(len(self.list))+1.0,c='k',label='chianti')
        ax.legend()
        fig.show()
    ...
    def plot_all_spectra(self,besti=6,resp=True):
        ...
        #get and plot response functions
        if resp:
            gresp=pickle.load(open('/Users/wheatley/Documents/Solar/occulted_flares/goes_resp.p','rb'))
            mresp=pickle.load(open('/Users/wheatley/Documents/Solar/occulted_flares/messenger_resp.p','rb'))
            glong=gresp['glong']
            gshort=gresp['gshort']
            longwn=gresp['longwn']
            shortwn=gresp['shortwn']
            drmts=mresp['drmts']
            eit=mresp['eit'][0]
            #normalize
            glong=glong/np.max(glong)
            gshort=gshort/np.max(gshort)
            drmts=drmts/np.max(drmts)


        fig,(ax,ax1)=plt.subplots(2,sharex=True, gridspec_kw={'height_ratios':[1,2]})
        for i,l in enumerate(lcs):
            if i !=besti:
                try:
                    ax1.plot(xaxes[i],yaxes[i],color='gray', linewidth='5')
                except valueError:
                    continue
        #print np.shape(xaxes), np.shape(yaxes),np.shape(xaxes[besti]),np.shape(yaxes[besti]),self.list[besti].Datetimes.Messenger_peak
        #ax1.plot(xaxes[besti],yaxes[besti],color='green', linewidth='2', label='June 1 2007 Flare')
        ax1.plot(xaxes[besti],yaxes[besti],color='r', linewidth='2', label='June 1 2007 Flare')
        ...
        #print 'here'
        #ax1.plot(np.linspace(1.5,12.4,25),.2*np.ones(25),color='blue',label='GOES 1-8 $\AA$',linewidth=2)
        #ax1.plot(np.linspace(3,25,25),.25*np.ones(25),color='green',label='SAX response',linewidth=2)
        #ax1.plot(np.linspace(3,25,25),.25*np.ones(25),color='cyan',label='GOES .5-4 $\AA$',linewidth=2)
        #print 'pre-resp'
        if resp:
            ax.plot(longwn[0],glong,color='blue',label='XRS 1-8 $\AA$', linewidth='2')
            ax.plot(shortwn[0],gshort, color='cyan',label='XRS 0.5-4 $\AA$', linewidth='2')
            ax.plot(eit[:-20],drmts[:-20], color='green',label='SAX', linewidth='2')
        ...
        ax1.set_ylabel('Messenger Count Rate (1.5-8 keV)')
        ax1.set_yscale('log')
        ax1.set_xscale('log')
