...
def norh_on_304(maplist17,file304='AIA/304/mapcube304_ext.p',moviename='norh_on_304.mp4',framerate=12,bsub=0,levels=[19,20,29,30,39,40],creverse=True):
    #maplist17.sort()
    #maplist34.sort()
    #build the pre-flare image for subtraction
    maplist304=pickle.load(open(file304,'rb'))
    times304=[dt.strptime(m.meta['date-obs'][:-3],'%Y-%m-%dT%H:%M:%S') for m in maplist304]
    bl17=maplist17.maps[:6]
    ...
    #make a of -1 (no rhessi) or index (rhessi) corresponding to NORH times
    rindex17=range(0,len(maplist304))#len(norhtimes17))
    current_idx=0
    next_idx=1
    ...
    for i,m304 in enumerate(maplist304):#[:len(submaps17)]):
        newdata=m304.data/m304.meta['exptime']
        newmap=sunpy.map.Map(newdata,m304.meta)
        del(m304)
        m304=newmap
        ...
        m304.plot_settings['cmap']=new_cmap
        imname='norh304_'+'{0:03d}'.format(i)+'.png'
        #fig,ax=plt.subplots()

        if rindex17[i] != -1: #make composite map with AIA
            #print(rindex17[i])
            comp_map17 = sunpy.map.Map(m304,submaps17[rindex17[i]], composite=True)
            comp_map17.set_alpha(0, 1)
            comp_map17.set_alpha(1, 1)
        ...
        #plt.clf()
    #mov.run_ffmpeg('norhmap'+'%03d.png',moviename,framerate=framerate)
    return compmaps#,maplist304,submaps17
...
def movie_from_groups(file_groups,moviename,framerate=12,imdir='.',nran=[0,-1],ext='png',submap=[False, False, False,False],overwrite=False,creverse=True,show=False,extrema=[[0,3500],[0,3500],[0,4000],[0,500]],normalize=True):
    idx=nran[0]
    #print('prefig', len(synced_maps))

    #ft=np.transpose(file_groups)
    for fg in file_groups[nran[0]:nran[1]]:
        imname=imdir+'/map_'+'{0:04d}'.format(idx)+'.'+ext
        previm=imdir+'/map_'+'{0:04d}'.format(idx-1)+'.'+ext
        if fg==file_groups[idx-1] and idx !=0 and os.path.isfile(previm):#the one before it
            #they are the same, just copy the plot
                copyfile(previm, imname)
                idx +=1
        else:
            sm=file_groups_to_synced([fg])
            if submap:
                  pms=[process_map(m,submap=submap[i],creverse=creverse,edges=False,pickle=False,normalize=normalize) for i,m in enumerate(sm[0])]
            else:
                pms=[process_map(m,creverse=creverse,edges=False,pickle=False,normalize=normalize) for i,m in enumerate(sm[0])]

            pms.reverse()#for when the AIA is on top ... put it at the bottom (where does this put norh?)
            pms[1],pms[2]=pms[2],pms[1]
            plot1_finalfig(pms,imname, extrema=extrema)
            idx +=1
    #make movie. add try and except if images not found...
    #mov.run_ffmpeg('map_'+'%03d.png',moviename,framerate=framerate)
    #print 'movie ', moviename, ' is stored in ', mdir
...
def plot1_finalfig(pms, imname,extrema =False, show=False):
    def my_func(x, pos):
        return str(x/1000)

    fmt1 = tk.FuncFormatter(my_func)
    #fig=plt.figure(figsize=(13,15))
    fig, ax = plt.subplots(2,2, figsize=(15,13),gridspec_kw = {'width_ratios':[1.4, 1]})
    for a,pm in enumerate(pms): # assume they are re-shuffled: stereo-B, stereo-A, 193+rhessi, 304+norh
        if extrema:
            norm=colors.Normalize(vmin=extrema[a][0],vmax=extrema[a][1])
        else:
            norm=colors.Normalize(vmin=np.min(pm.data),vmax=np.max(pm.data))
        ...
        if a <2:
            ...
            lax=ax[1][a-2]
        #ax=fig.add_subplot(2,2,a+1) #2 1 1 , 2 1 2, 2 2 1, 2 2 2 ?
        if type(pm) !=sunpy.map.compositemap.CompositeMap:
            cf=pm.plot(norm=norm,axes=lax) #later add option for colormap normalization
        else:
            p0=pm.get_plot_settings(0)
            p0['norm']=norm=colors.Normalize(vmin=extrema[a][0],vmax=extrema[a][1])#vmin=np.min(pm._maps[0].data),vmax=np.max(pm._maps[0].data))
            pm.set_plot_settings(0,p0)
            print(lax.get_position().bounds)
            cf=cmap_plot(pm,axes=lax)[0]
             #pm.plot(axes=lax)
        lax.yaxis.label.set_visible(False)
        lax.set_xlabel('Solar X (arcsec)')#yaxis.label.set_visible(False)

        if a == 0:
            cbaxes = fig.add_axes([0.54, 0.55, 0.015, 0.34])
            cbar=fig.colorbar(cf,cax=cbaxes,format=fmt1)
            cbar.set_label('x1000')
            lax.set_xlim([250,600])
            lax.set_ylim([100,350])
            lax.set_position([0.12499999999999999, 0.51176423186188802, 0.4057579234182096, 0.41341573234346685])
        elif a == 1:
            lax.set_xlim([-1100,-850])
            lax.set_ylim([100,350])
            cbaxes = fig.add_axes([0.935, 0.55, 0.015, 0.34])
            cbar193=fig.colorbar(cf,cax=cbaxes,format=fmt1)
            #lax.set_position([.55,.558,.28,.28])
            #    print(lax.get_position().bounds)
            try:
                lax.set_title('AIA $193\: \AA$ 2013-05-01 '+pm._maps[0].date.strftime('%H:%M:%S') )
            except AttributeError:
                pass
            cbar193.set_label('x1000')
            #print(lax.get_position().bounds)
            #lax.set_position([0.63, 0.51176423186188802, 0.3133265817901234, 0.41341573234346695])
            lax.set_position([0.63, 0.53636363636363638, 0.29356060606060597, 0.36363636363636365])
            #print(lax.get_position().bounds)
        elif a == 2:
            cbaxes = fig.add_axes([0.54, 0.11, 0.015, 0.34])
            cbar=fig.colorbar(cf,cax=cbaxes,format=fmt1)
            cbar.set_label('x1000')
            lax.set_xlim([900,1250])
            lax.set_ylim([250,500])
            lax.set_position([0.12499999999999999, 0.0706583771407987, 0.4057579234182096, 0.4134157323434669])
        else:
            cbaxes = fig.add_axes([0.935, 0.11, 0.015, 0.34])
            cbar=fig.colorbar(cf,cax=cbaxes,format=fmt1)
            cbar.set_label('x1000')
            try:
                lax.set_title('AIA $304\: \AA$ 2013-05-01 '+pm._maps[0].date.strftime('%H:%M:%S') )
            except AttributeError:
                pass
            lax.set_xlim([-1100,-850])
            lax.set_ylim([100,350])
            lax.set_position([0.63, 0.095,0.29356060606060597, 0.36363636363636365])

        #fig.subplots_adjust(bottom=.7-(float(a)/3.))
    #fig.tight_layout()
    #print(ax[0][0].get_position().bounds)
    #print(ax[0][1].get_position().bounds)
    #print(ax[1][0].get_position().bounds)
    #print(ax[1][1].get_position().bounds)
...
def group_lists(all_lists, cadence=1):
    nlists=np.shape(all_lists)[0]
    basetimevec=time_vecs(all_lists[0])
    otv=[time_vecs(l) for l in all_lists]
    groups=[]
    st=basetimevec[0]
    nseconds=int((basetimevec[-1]-st).total_seconds())
    vlen=int(nseconds/cadence +1)
    abs_tvec=[st+td(seconds=cadence+i) for i in range(0,vlen)]
    #return abs_tvec
    ...
    for tv in abs_tvec:
        ...
        for i in range(0,nlists):
            ...
            if otv[i][nextv] > tv:
                try:
                    cgroup.append(all_lists[i][nextv-1])
                except IndexError:
                    cgroup.append(all_lists[i][0])
            else:
                cgroup.append(all_lists[i][nextv])
        groups.append(cgroup)

    return groups

def dtgt(dt_in, reftime):
    vv=[dtc for dtc in dt_in if (dtc-reftime).total_seconds() >=0]
    try:
        return dt_in.index(min(vv))
    except ValueError:
        return 0

def file_groups_to_synced(fg):
    if type(fg) ==str:
        file_groups=pickle.load(open(fg,'rb'))
    else:
        file_groups = fg
    os.chdir('../../AIA/high_cadence_cutout')
    synced_maps=[]
    for i, cf in enumerate(file_groups):
        gmaps=[]
        ...
        #now restore the maps
        for ci,c in enumerate(cf):
            if c.endswith('A.p'): #make sure the ddir info is carried over with c
                os.chdir('../../STEREO/A')
                newmap=pickle.load(open(c,'rb'))
                os.chdir('../../AIA/high_cadence_cutout')
            elif c.endswith('B.fts'):
                os.chdir('../../STEREO/B')
                newmap=sunpy.map.Map(c)
                os.chdir('../../AIA/high_cadence_cutout')
            elif c.startswith('compmap'):
                os.chdir('../../Xray')
                newmap=pickle.load(open(c,'rb'))
                os.chdir('../AIA/high_cadence_cutout')
            elif c.startswith('norhmap'):
                os.chdir('../../NORH')
                newmap=pickle.load(open(c,'rb'))
                os.chdir('../AIA/high_cadence_cutout')
            else:
...
def time_vecs(namevec,norh=False):
    timevec=[]
    for n in namevec:
        if n.startswith('compmap'):
            timevec.append(dt.strptime(n[8:-6],'%Y%m%d_%H%M%S'))
        elif n.startswith('norhmap'):
            timevec.append(dt.strptime(n[8:-3],'%Y%m%d_%H%M%S'))
        elif n.endswith('.p'):
            timevec.append(dt.strptime(n[-23:-8],'%Y%m%d_%H%M%S')) #-21,-8
        elif norh:
            timevec.append(dt.strptime(n[3:],'%y%m%d_%H%M%S'))
        elif n.startswith('AIA'):
            timevec.append(dt.strptime(n[8:-5],'%Y%m%d_%H%M%S'))
        else:
            timevec.append(dt.strptime(n[:-10],'%Y%m%d_%H%M%S')) #8,-5
    return timevec
...
def process_map(smap,submap=False,creverse=False,edges=False,pickle=False,normalize=True):
    ...
    if type(smap) == sunpy.map.compositemap.CompositeMap:
        return smap
    if submap:
        bl=submap[0]
        tr=submap[1]
    ...
    if normalize:
        if mapss.meta['exptime'] > 8.0: #this really should only be the case for STEREO
            ...
            del(mapss)
            mapss=newmap
        if mapss.meta['instrume'].startswith('AIA'):
            newdata=mapss.data/mapss.meta['exptime']
            newmap=sunpy.map.Map(newdata,mapss.meta)
            del(mapss)
            mapss=newmap
