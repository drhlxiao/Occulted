...
def fit_rhessi_decay(xsi_rhessi='plot_curves.sav',maxidx=20):
    from scipy.io import readsav
    xrdict=readsav(xsi_rhessi,python_dict=True)
    hsi_time=xrdict['hsi_time']
    hsi12=xrdict['hsi12']/np.max(xrdict['hsi12'])
    hsi_dt=[dt.strptime(ht.lstrip()[:-4], '%d-%b-%Y %H:%M:%S') for ht in hsi_time]
    epoch = dt(2013,5,1)
    hsi_float=[t for t in [(d - epoch).total_seconds() for d in hsi_dt]]
    peakidx=np.where(hsi12 == np.max(hsi12))[0][0]
    fity=hsi12[peakidx:peakidx+maxidx] #or 20
    fitx=range(0,maxidx)#hsi_float[peakidx:peakidx+maxidx]
    p=scipy.optimize.curve_fit(lambda t,a,b: a*np.exp(-1.*b*t),  fitx,  fity)
    #p=np.polyfit(fitx,fity,2,w=w)
    #pcurve=np.poly1d(p)
    print(p[0])#, np.shape(pcurve))

    #plot fit
    fig,ax=plt.subplots(figsize=(6,6))
    ax.step(hsi_dt[peakidx:peakidx+maxidx],fity,'b-',label='RHESSI 10-30 keV',linewidth=2)
    ax.plot_date(hsi_dt[peakidx:peakidx+maxidx],p[0][0]*np.exp(-1.*p[0][1]*np.array(range(0,maxidx))),'r-')
    ax.plot()
    myFmt = DateFormatter('%H:%M')
    ax.xaxis.set_major_formatter(myFmt)
    plt.gcf().autofmt_xdate()
    ax.set_xlabel('Start Time (01-May-2013 01:30:00 UTC)')
    ax.set_ylabel('Normalized Flux')
    ax.set_xlim([hsi_dt[40],hsi_dt[-10]])
    ax.set_ylim([-.5,1.1])
    ax.legend(loc='lower right')#,fontsize='small')
    #etc
    fig.show()
...
def figure2(returnp=False):
    ...
    dtv94=dtv94[85:225]
    dtvFe18=dtvFe18[85:225]
    lc94=lc94[85:225]
    lcFe18=lcFe18[85:225]


    fig,ax=plt.subplots(2,sharex=True,figsize=(9,10))
    ...
    ax[0].plot_date(tm_dt,tm_flux[0],'c+-',label='SXI TM')
    ax[0].plot_date(be_dt,be_flux[0],'r+-',label='SXI Be')
    ax[0].fill_between(dtv94[60:75],1.1,-.5,alpha=0.3,color='grey')
    ax[0].axvline(x=dtv94[68], linestyle='dashed',color='k')
    #ax[0].plot_date(dtvNORH,(lcNORH-lcNORH[7])/np.max(lcNORH-lcNORH[7]),'k.-',label='NORH 17 GHz')
    ax[0].set_ylabel('Normalized Flux')
    ax[0].set_ylim([-.5,1.1])
    ax[0].legend(loc='upper left')#,fontsize='small')

    ax[1].plot_date(sB_time,sB_curve,'b.-',label='STEREO-B 195')#,where='mid')
    ax[1].plot_date(sA_time,sA_curve,'m.-',label='STEREO-A 195')#,where='mid')
    ...
    ax[1].fill_between(dtv94[60:75],1.1,-3.5,alpha=0.3,color='grey')
    ax[1].axvline(x=dtv94[68], linestyle='dashed', color='k')
    ...
    plt.gcf().autofmt_xdate()
    ax[1].set_xlabel('Start Time (01-May-2013 01:30:00 UTC)') #why does this not show up? subplots_adjust...
...
def goes_triple():
    ...
    fig,ax=plt.subplots(2,sharex=True,figsize=(8,8)) #aspect ratio
    ax[0].plot_date(xrsa_t[l:u],xrsa[l:u], 'b-')
    ax[0].plot_date(xrsb_t[l:u],xrsb[l:u], 'r-')
    ax[0].set_yscale('log')
    #ax[1].plot_date(xrsa_t[l:u],xrsa[l:u],'b-') #do I want these to have a similar y-axis range for comparison?
    ax[1].plot_date(xrsb_t[l:u],xrsb[l:u],'r-')
